...
class BinaryConditionalFlowMatcher:
    """Base class for conditional flow matching methods. This class implements the independent
    conditional flow matching methods from [1] and serves as a parent class for all other flow
    matching methods.
    ...
    def guided_sample_location_and_conditional_flow(
            ...
            matches = torch.all( x0[:, -2:] == x1[:, -2:] , dim=1)  # Assuming last two columns are labels
            x0 = x0[matches]
            x1 = x1[matches]
            t = t[matches]
            y0 = y0[matches] if y0 is not None else None
            y1 = y1[matches] if y1 is not None else None
...
class ExactOptimalTransportConditionalFlowMatcher(ConditionalFlowMatcher):
    ...
    def __init__(self, sigma: Union[float, int] = 0.0):
        ...
        self.ot_sampler = OTPlanSampler(method="exact")#, normalize_cost=False, reg=5)
...
class UnbalancedOptimalTransportConditionalFlowMatcher(ConditionalFlowMatcher):
    """Child class for optimal transport conditional flow matching method. This class implements
    the OT-CFM methods from [1] and inherits the ConditionalFlowMatcher parent class.
    ...
    def __init__(self, sigma: Union[float, int] = 0.0):
        ...
        """
        super().__init__(sigma)
        self.ot_sampler = OTPlanSampler(method="unbalanced", normalize_cost=False, reg=0.05, reg_m=(0.01, 0.01))
    ...
    def guided_sample_location_and_conditional_flow(
        ...
        [1] Improving and Generalizing Flow-Based Generative Models with minibatch optimal transport, Preprint, Tong et al.
        """
        x0, x1, y0, y1 = self.ot_sampler.sample_plan_with_labels(x0, x1, y0, y1, replace=False)
        if return_noise:
            t, xt, ut, eps = super().sample_location_and_conditional_flow(x0, x1, t, return_noise)
            return t, xt, ut, y0, y1, eps
        else:
...
class SommerConditionalFlowMatcher(ConditionalFlowMatcher):
    """Child class for optimal transport conditional flow matching method. This class implements
    the OT-CFM methods from [1] and inherits the ConditionalFlowMatcher parent class.
    ...
    def __init__(self, sigma: Union[float, int] = 0.0):
        ...
        """
        super().__init__(sigma)
        self.ot_sampler = OTPlanSampler(method="unbalanced", normalize_cost=True, reg=0.05, reg_m=(0.01, 0.01))
    ...
    def guided_sample_location_and_conditional_flow(
        ...
        [1] Improving and Generalizing Flow-Based Generative Models with minibatch optimal transport, Preprint, Tong et al.
        """
        x0, x1, y0, y1 = self.ot_sampler.sample_sommer(x0, x1, y0, y1)
...
class WeightedOptimalTransportConditionalFlowMatcher(ConditionalFlowMatcher):
    """Child class for optimal transport conditional flow matching method. This class implements
    the OT-CFM methods from [1] and inherits the ConditionalFlowMatcher parent class.
    ...
    def guided_sample_location_and_conditional_flow(
        ...
        [1] Improving and Generalizing Flow-Based Generative Models with minibatch optimal transport, Preprint, Tong et al.
        """
        x0, x1, y0, y1 = self.ot_sampler.sample_weighted_plan_with_labels(x0, x1, y0, y1)
        if return_noise:
            t, xt, ut, eps = super().sample_location_and_conditional_flow(x0, x1, t, return_noise)
            return t, xt, ut, y0, y1, eps
        else:
